...
# the natural growth rate of rabbits(Species 1)
a = 1
# death rate of rabbits due to predators(species 2)
b = 0.1
# natural death rate of predator when there are no prey( natural death rate of species 2 when there is no specie 1)
c = 1.5
# the facor describing how many predator(Species 2) eat the prey(species 1)
d = 0.75
...
#ODE Variable for Mutualism
# the natural growth rate of (Species 1)
a = 1
# death rate of rabbits due to predators(species 2)
b = 0.1
# natural death rate of (species 2) when there are no (species 1)
# the facor describing how many (Species 2) coexist with(species 1)
d = 0.75
...
# They are imaginary numbers so that species 1 and 2 are periodic and the periods are given by
T_f1 = 2 * pi / abs(lambda1)
...
# initial conditions, 10 species 2 and 5 species 1
X0 = array([10, 5])
...
#function to calculate effective population of rabbit
def effectivePopSpecies1(X,t):
   summation = 0

   for i in range(0, t.size):
       summation += 1 / species1[i]
       X[i] = float((1 / (t[i] * float(summation))))
    #returns ne_r
   return X
...
#function to calculate effective population of wolves
def effectivePopSpecies2(X,t):
   summation = 0

   for i in range(0, t.size):
       summation += 1 / species2[i]
...
#Fills ne_r and new_w with effective populations for rabbits and wolves
ne_r = effectivePopSpecies1(ne_r,t)
ne_w = effectivePopSpecies2(ne_w,t)
